x<-c(0.21,0.10,0.13,0.56)#reference sample
a<-c(x,rnorm(3,mean=mean(x),sd=sd(x)))
no_of_comparisons<-15 #how many comparisons?
...
alpha #Sidak-adjusted alpha
quantile(dx,alpha/2)
sigma #estimated sigma
i #estimated number of samples (two-sided test)
5*i+length(a)
